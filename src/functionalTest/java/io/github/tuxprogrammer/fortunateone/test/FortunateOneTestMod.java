...
@Mod(
    modid = "fortunateone-tests",
    name = "Fortunate One Dev Tests",
    version = "1.0",
    dependencies = "required-after:fortunateone",
    acceptableRemoteVersions = "*")
public class FortunateOneTestMod {

    @Mod.EventHandler
    public void onServerStarted(FMLServerStartedEvent event) {
        MinecraftServer.getServer()
            .addChatMessage(new ChatComponentText("Running Fortunate One integration tests..."));
        runTests();
        MinecraftServer.getServer()
            .addChatMessage(new ChatComponentText("Fortunate One integration tests finished"));
    }
    ...
    private void runTests() {
        ...
        final LauncherDiscoveryRequest discovery = LauncherDiscoveryRequestBuilder.request()
            .selectors(DiscoverySelectors.selectPackage("io.github.tuxprogrammer.fortunateone.test"))
            .build();
        ...
        if (summary.getTotalFailureCount() > 0 && FMLCommonHandler.instance()
            .getSide()
            .isServer()) {
            throw new RuntimeException("Fortunate One integration tests failed — check the log for details");
        }
    }
}
